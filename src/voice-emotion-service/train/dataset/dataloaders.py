...
def create_dataloaders(
    dataset: SpecDataset, model_config: ModelConfig, aug_config: AugmentsConfig
):
    # Split Actors, not Samples
    all_actors = list(set(s.actor for s in dataset.samples))
    train_actors, temp_actors = train_test_split(
        all_actors, test_size=0.2, random_state=42
    )
    val_actors, test_actors = train_test_split(
        temp_actors, test_size=0.5, random_state=42
    )

    print(
        f"Split Stats (Actors): Train={len(train_actors)}, Val={len(val_actors)}, Test={len(test_actors)}"
    )
    ...
    print(
        f"Split Stats (Samples): Train={len(train_samples)}, Val={len(val_samples)}, Test={len(test_samples)}"
    )
    ...
    train_loader = DataLoader(
        train_ds, batch_size=model_config.batch_size, shuffle=True, num_workers=2
    )
    val_loader = DataLoader(
        val_ds, batch_size=model_config.batch_size, shuffle=False, num_workers=2
    )
    test_loader = DataLoader(
        test_ds, batch_size=model_config.batch_size, shuffle=False, num_workers=2
    )
